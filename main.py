...
def sample_images(batches_done):
    ...
    disaster = torch.randint(0, 3, (1,))
    fake_B = generator(real_A, disaster)
    img_sample = torch.cat((real_A.data, fake_B.data, real_B.data), -2)
    save_image((img_sample * 0.5) + 0.5, "images/%s.png" % batches_done, nrow=5, normalize=True)
...
def main():
    ...
    for e in range(n_epochs):
        for i, (pre_images, post_images, label) in enumerate(dataloader):
            ...
            valid = Variable(Tensor(np.ones((pre_images.size(0), *patch))), requires_grad=False)
            fake = Variable(Tensor(np.ones((post_images.size(0), *patch))), requires_grad=False)

            optimizer_G.zero_grad()
            gen_labels = Variable(torch.LongTensor(np.random.randint(0, 3, batch_size)))
